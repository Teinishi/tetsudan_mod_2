...
export_utils.collection_export("downlight", export_path, "m_tns_")
...
for type_name in [f"type{i}" for i in range(1, 6)]:
    for suffix in ("_base", "_light"):
        ...
        export_utils.export_objects(
            [obj], f"m_tns_interior_light_{type_name}_5{suffix}.dae", dirname=export_path)
        ...
        export_utils.translate_vertices(
            dup_obj, (0, 0.25, 0), lambda c: c[1] <= -0.5)
        export_utils.export_objects(
            [dup_obj], f"m_tns_interior_light_{type_name}_3{suffix}.dae", dirname=export_path)
